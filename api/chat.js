...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      temperature: 0.85,
      max_tokens: 500,
      // Reasoning-capable models (Qwen3.x, GPT-OSS) can emit an internal "thinking"
      // trace alongside the actual reply. These settings tell Groq to suppress it.
      // Qwen models: 'none' disables reasoning entirely. GPT-OSS models ignore
      // 'none' for reasoning_effort but respect include_reasoning:false.
      reasoning_effort: MODEL.includes("qwen") ? "none" : undefined,
    ...
    // Safety net: some reasoning models occasionally leak a raw <think>...</think>
    // block into the content field despite the settings above. Strip it if present,
    // keeping only whatever comes after the closing tag (the actual reply).
    if (reply && reply.includes("<think>")) {
      const closeIdx = reply.indexOf("</think>");
      reply = closeIdx !== -1
        ? reply.slice(closeIdx + "</think>".length).trim()
        : reply.replace(/<think>[\s\S]*/i, "").trim();
    }
